...
4. После чего следует удалить из проекта пути к модуляем, которые не были залиты на GitHub, если такие имеются. Также рекомендую перенести в *.env* TOKEN вашего бота, чтобы никто не мог его увидеть, а в самом коде прописать `const token = process.env.TOKEN;`. При этом token - переменная, котрую вы используете в своем коде, а TOKEN - имя переменной из *.env*.  
